function [x, error, iter, flag] = gmresGramSchmidt(jx_func,b,restrt,tol,max_it,x,u0,scalars)
...
%                           1 = no convergence given max_it
   ...
   for iter = 1:max_it,                              % begin iteration
	 ...
	 w = M \ (jx_func(V(:,i),u0,scalars));                         % basis using Gram-Schmidt
	 for k = 1:i,
	   H(k,i)= w'*V(:,k);
	   w = w - H(k,i)*V(:,k);
	 end
	 ...
	 if ( error <= tol ),                        % update approximation
      ...
      r = M \ ( b-jx_func(x,u0,scalars) );                              % compute residual
      s(i+1) = norm(r);
